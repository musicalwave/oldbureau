...
// onselect is more specified than onchange, and is triggered only when the user
// makes a specific selection
...
DatePicker.prototype.create = function(doc) {
	...
	this._todayButton.appendChild(doc.createTextNode("Сегодня"));
	...
	doc.onclick = function(e) {
		...
		if (Tapestry.DateField.activeDateField != null) {
			...
			if (insideDatePicker == null
					&& targ.className != 't-calendar-trigger') {
...
DatePicker.prototype.setDate = function(oDate, forceOnSelect) {
	...
	if (oDate == null) {
		...
		// but if it /is/ null, we have to see if we were inited or not. If we
		// weren't inited, then we're
	...
	if (this._selectedDate == null
			|| !this._datesAreSame(this._selectedDate, oDate)) {
		this._selectedDate = new Date(oDate);

		if (typeof this.onchange == "function")
			this.onchange();

		// so if _selectedInited is false, then the value is different only
		// because we set the value programmatically, post-initialization.
		// that handles the creation + set event. Subsequent reveals will set it
		// to whatever _selectedDate already was, so it's handled.
...
DatePicker.prototype._updateTable = function() {
	...
	var tmpDate = new Date(this._calendarDate.getFullYear(), this._calendarDate
			.getMonth(), 1);
	var today = new Date();
	...
	for (i = 1; i < 32; i++) {
		...
		if (tmpDate.getMonth() == this._calendarDate.getMonth()) {
			...
			cells[currentWeek][colIndex].text = this._matrix[currentWeek][colIndex] = tmpDate
					.getDate();
	...
	// check if there days in the last week
	var cell = this._matrix[rows - 1][0];
	var hasWeek6 = cell != null;
	if (!hasWeek6)
		delete this._matrix[rows - 1];
}

// sets the label showing the year and selected month
DatePicker.prototype._setTopLabel = function() {
	var str = this._calendarDate.getFullYear() + " "
			+ DatePicker.months[this._calendarDate.getMonth()];
...
DatePicker.prototype.goToNextMonth = function() {
	...
	d.setDate(Math.min(d.getDate(), DatePicker.getDaysPerMonth(
			d.getMonth() + 1, d.getFullYear()))); // no need to catch dec ->
	// jan for the year
...
DatePicker.prototype.goToPreviousMonth = function() {
	var d = new Date(this._calendarDate);
	d.setDate(Math.min(d.getDate(), DatePicker.getDaysPerMonth(
			d.getMonth() - 1, d.getFullYear()))); // no need to catch jan ->
	// dec for the year
	d.setMonth(d.getMonth() - 1);
	this.setCalendarDate(d);
}
...
DatePicker.prototype.goToToday = function() {
	// note: small tweak here so that clicking the "Today" button will properly
	// update the selected date and trigger selected
	// but note that we want this behavior iff "today" is already selected and
	// visible.
	// For instance: If you're looking at some date months away from today and
	// want to jump back to today AND today is the selectedDate
	// then we don't want that to close the calendar.
	var today = new Date();
	var forceOnSelect = false;
	if (this._selectedDate == null
			|| (this._datesAreSame(today, this._selectedDate)
					&& this._calendarDate.getMonth() == today.getMonth() && this._calendarDate
					.getFullYear() == today.getFullYear())) {
		// then go ahead and force the selection...
		forceOnSelect = true;
	}
	this.setDate(new Date(), forceOnSelect);// note that setDate calls
	// setCalendarDate...
}
...
DatePicker.prototype._showLabelPopup = function() {
	...
	for ( var i = -3; i < 4; i++) {
		...
		// year change when checking
		// leap
		tmp2.setDate(1);
		tmp2.setMonth(tmp2.getMonth() + i);
		tmp.setDate(Math.min(tmp.getDate(), DatePicker.getDaysPerMonth(tmp
				.getMonth()
				+ i, tmp2.getFullYear())));
